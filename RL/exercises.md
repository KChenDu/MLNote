...
5. **假设你将杆平衡作为一个情节性任务，但是也使用了衰减因子，除了-1是失败之外，所有奖励都是零。那么每次回报是多少？这个回报与有衰减的持续任务有什么不同？**
...
1. **证明$\sum_{k = 0}^\infty\gamma^k = \frac1{1 - \gamma}$。**
2. **如果当前状态为$S_t$，并且根据随机策略$\pi$选择动作，则对于$\pi$和四参数函数$p$，$R_{t + 1}$的期望是多少？**
