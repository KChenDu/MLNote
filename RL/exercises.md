...
7. **例的结果及其展示图中，我们使用了首次访问MC方法。假设对同样的问题，我们使用每次访问MC方法。估计值的方差还会是无限吗？为什么？**
8. **写出从$\rho_{t:T - 1}R_{t + 1} = \frac{\pi(A_t | S_t)}{b(A_t | S_t)}\frac{\pi(A_{t + 1} | S_{t + 1})}{b(A_{t + 1} | S_{t + 1})}\frac{\pi(A_{t + 2} | S_{t + 2})}{b(A_{t + 2} | S_{t + 2})}\dots\frac{\pi(A_{T - 1} | S_{T - 1})}{b(A_{T - 1} | S_{T - 1})}R_{t + 1}$导出$\mathbb E[\rho_{t:T - 1}R_{t + 1}] = \mathbb E[\rho_{t:t}R_{t + 1}]$的步骤。**
