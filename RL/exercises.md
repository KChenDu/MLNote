...
2. **该参数$\lambda$表征图中的指数加权下降的速度，以及因此$\lambda$回报算法在确定其更新时所看到的未来的距离。但是速率因素比如$\lambda$有时是表征衰变速度的尴尬方式。出于某些目的，最好指定时间常数或半衰期。什么是与$\lambda$和半衰期$\mathcal T_\lambda$，即加权序列将落到其初始值的一半的时间相关的等式？**